...
export const DirectoryExplorerDialog: React.FC<DirectoryExplorerDialogProps> = ({
    ...
    try {
      ...
      if (isDesktop) {
        ...
        if (!accessResult.success) {
          toast.error(t('session.unableToAccessDirectory'), {
        ...
        if (!startResult.success) {
          toast.error(t('session.failedToOpenDirectory'), {
      ...
      if (!added) {
        toast.error(t('session.failedToAddProject'), {
      ...
      toast.error(t('session.failedToSelectDirectory'), {
  ...
  const pathInputSection = (
    <div className="relative">
      <Input
        ...
        placeholder={t('session.enterPathOrSelect')}
  ...
  const renderActionButtons = () => (
    <>
      ...
      >
        {isConfirming ? t('session.adding') : t('session.addProject')}
      </Button>
    </>
  );
  ...
  if (isMobile) {
    return (
      <MobileOverlayPanel
        ...
        title={t('session.addProjectDirectory')}

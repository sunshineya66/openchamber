  ...
  return (
    <div className={cn('flex h-full flex-col', bgClass)}>
      <div className="border-b px-3 pt-4 pb-3">
        <h2 className="text-base font-semibold text-foreground mb-3">{t('settings.usage.title')}</h2>
        <div className="flex items-center justify-between gap-2">
          <span className="typography-meta text-muted-foreground">{t('settings.usage.total', { count: QUOTA_PROVIDERS.length })}</span>
          <div className="flex items-center gap-2">
              ...
              <TooltipTrigger asChild>
                <span className="inline-flex">
                  <Checkbox
                    ...
                    ariaLabel={t('settings.usage.autoRefresh')}
                  />
                </span>
              </TooltipTrigger>
              <TooltipContent side="bottom">
                {t('settings.usage.autoRefreshHint')}
              </TooltipContent>
            </Tooltip>
            <Select
            ...
            >
              <SelectTrigger className="w-fit">
                <SelectValue placeholder={t('settings.usage.interval')} />
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          <span className="typography-micro text-muted-foreground">{t('settings.usage.display')}</span>
          <Select value={usageDisplayMode} onValueChange={handleUsageDisplayModeChange}>
            <SelectTrigger className="w-fit">
              <SelectValue placeholder={t('settings.usage.displayMode')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="usage">{t('settings.usage.displayUsage')}</SelectItem>
              <SelectItem value="remaining">{t('settings.usage.displayRemaining')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
        ...
        {QUOTA_PROVIDERS.map((provider) => {
          ...
          return (
            ...
            >
              ...
              {!configured && (
                <span className="typography-micro text-muted-foreground/60 flex-shrink-0">{t('settings.usage.notSet')}</span>
              )}
            </button>
          </div>
          );

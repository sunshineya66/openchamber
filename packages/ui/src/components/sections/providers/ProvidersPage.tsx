...
export const ProvidersPage: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const loadAuthMethods = async () => {
      ...
      try {
        ...
        toast.error(t('features.providers.loadAuthMethodsError'));
      } finally {
        if (isMounted) {
  ...
  React.useEffect(() => {
    ...
    const loadAvailableProviders = async () => {
      ...
      try {
        ...
        setAvailableError(t('features.providers.loadError'));
      } finally {
        if (isMounted) {
  ...
  const handleSaveApiKey = async (providerId: string) => {
    ...
    try {
      ...
      toast.success(t('settings.toast.saveSuccess', 'API key saved'));
      setApiKeyInputs((prev) => ({ ...prev, [providerId]: '' }));
      await reloadOpenCodeConfiguration({ scopes: ["providers"], mode: "active" });
      setSelectedProvider(providerId);
    } catch (error) {
      console.error('Failed to save API key:', error);
      toast.error(t('features.providers.saveApiKeyError'));
    } finally {
      setAuthBusyKey(null);
    }
  };
  ...
  const handleOAuthStart = async (providerId: string, methodIndex: number) => {
    ...
    try {
      ...
      toast.message(t('features.providers.oauthCompleteHint', 'Complete the OAuth flow in your browser'));
    } catch (error) {
      console.error('Failed to start OAuth flow:', error);
      toast.error(t('features.providers.oauthStartError'));
    } finally {
      setAuthBusyKey(null);
    }
  };
  ...
  const handleOAuthComplete = async (providerId: string, methodIndex: number) => {
    ...
    try {
      ...
      toast.success(t('features.providers.oauthSuccess', 'OAuth connection completed'));
      ...
      toast.error(t('features.providers.oauthCompleteError'));
    } finally {
      setAuthBusyKey(null);
    }
  };
  ...
  const handleCopyOAuthLink = async (url: string) => {
    ...
    if (result.ok) {
      toast.success(t('features.providers.oauthLinkCopied', 'OAuth link copied'));
      return;
    }
    console.error('Failed to copy OAuth link:', result.error);
    toast.error(t('features.providers.copyOAuthLinkError'));
  };

  const handleCopyOAuthCode = async (code: string) => {
    const result = await copyTextToClipboard(code);
    if (result.ok) {
      toast.success(t('features.providers.codeCopied', 'Device code copied'));
      return;
    }
    console.error('Failed to copy device code:', result.error);
    toast.error(t('features.providers.copyCodeError', 'Failed to copy device code'));
  };
  ...
  const handleDisconnectProvider = async (providerId: string) => {
    ...
    try {
      ...
      toast.success(t('features.providers.disconnected'));
      await reloadOpenCodeConfiguration({ scopes: ["providers"], mode: "active" });
    } catch (error) {
      console.error('Failed to disconnect provider:', error);
      toast.error(t('features.providers.disconnectError'));
    } finally {
      setAuthBusyKey(null);
    }
  };
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center text-muted-foreground">
          ...
          <p className="typography-body">{t('features.providers.noProvidersDetected')}</p>
          <p className="typography-meta mt-1 opacity-75">{t('features.providers.checkConfiguration')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <ScrollableOverlay keyboardAvoid outerClassName="h-full" className="w-full">
        <div className="mx-auto w-full max-w-3xl p-3 sm:p-6 sm:pt-8">
          <div className="mb-4">
            <h1 className="typography-ui-header font-semibold text-foreground">{t('features.providers.connectProvider')}</h1>
          </div>

          <div className="mb-8">
            <div className="mb-1 px-1">
              <h2 className="typography-ui-header font-medium text-foreground">{t('features.providers.selectProvider')}</h2>
            </div>

            <section className="px-2 pb-2 pt-0">
              <div className="flex flex-wrap items-center gap-2 py-1.5">
                <span className="typography-ui-label text-foreground">{t('features.providers.provider')}</span>
                  {availableLoading ? (
                    <p className="typography-meta text-muted-foreground">{t('features.providers.loading')}</p>
                  ) : availableError ? (
                    <p className="typography-meta text-muted-foreground">{availableError}</p>
                  ) : unconnectedProviders.length === 0 ? (
                    <p className="typography-meta text-muted-foreground">{t('features.providers.allConnected')}</p>
                  ) : (
                          ...
                          <span className="flex items-center gap-2 min-w-0">
                            ...
                            <span className={cn("truncate typography-ui-label font-normal", candidateProviderId ? "text-foreground" : "text-muted-foreground")}>
                              {candidateProviderId
                                ...
                                : t('features.providers.selectProvider', 'Select provider')}
                            </span>
                          </span>
                      ...
                      >
                        ...
                        >
                          ...
                          <input
                            ...
                            onKeyDown={(e) => e.stopPropagation()}
                            placeholder={t('features.providers.search', 'Search...')}
                        ...
                        <ScrollableOverlay outerClassName="max-h-[240px]" className="p-1">
                          {(() => {
                            ...
                            if (filtered.length === 0) {
                              return <p className="py-4 text-center typography-meta text-muted-foreground">{t('features.providers.noProvidersFound', 'No providers found')}</p>;
                            }
          ...
          {candidateProviderId && (
            <div className="mb-8">
              <div className="mb-1 px-1">
                <h2 className="typography-ui-header font-medium text-foreground">{t('features.providers.authentication')}</h2>
              </div>

              {authLoading ? (
                <p className="typography-meta text-muted-foreground px-2">{t('features.providers.loadingAuthMethods', 'Loading authentication methods...')}</p>
              ) : (
                <section className="px-2 pb-2 pt-0 space-y-4">
                  <div className="py-1.5">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-1.5">
                      ...
                      >
                        {authBusyKey === `api:${candidateProviderId}` ? t('features.providers.saving') : t('features.providers.saveKey')}
                      </ButtonSmall>
                    </div>
                  </div>
                            ...
                            <div key={`${candidateProviderId}-${methodLabel}`} className="space-y-3">
                              ...
                              {oauthDetails[codeKey]?.userCode && (
                                <div className="flex items-center gap-2 mt-2">
                                  ...
                                  <ButtonSmall variant="outline" size="xs" className="!font-normal" onClick={() => handleCopyOAuthCode(oauthDetails[codeKey]?.userCode ?? '')}>{t('features.providers.copyCode')}</ButtonSmall>
                                </div>
                              )}
                              ...
                              {oauthDetails[codeKey]?.url && (
                                <div className="flex items-center gap-2 mt-2">
                                  ...
                                  <div className="flex gap-1 shrink-0">
                                    <ButtonSmall variant="outline" size="xs" className="!font-normal" onClick={() => window.open(oauthDetails[codeKey]?.url, '_blank', 'noopener,noreferrer')}>{t('features.providers.open')}</ButtonSmall>
                                    <ButtonSmall variant="outline" size="xs" className="!font-normal" onClick={() => handleCopyOAuthLink(oauthDetails[codeKey]?.url ?? '')}>{t('features.providers.copy')}</ButtonSmall>
                                  </div>
                                </div>
                              )}
                              ...
                              {isPending && (
                                <div className="flex items-center gap-2 mt-2">
                                  <Input
                                    ...
                                    placeholder={t('features.providers.pasteAuthCode')}
                                  ...
                                  >
                                    {authBusyKey === `oauth-complete:${candidateProviderId}:${index}` ? t('features.providers.saving') : t('features.providers.complete')}
                                  </ButtonSmall>
                                </div>
                              )}
                            </div>
  ...
  if (!selectedProvider) {
    return (
      <ScrollableOverlay keyboardAvoid outerClassName="h-full" className="w-full">
        <div className="mx-auto w-full max-w-3xl p-3 sm:p-6 sm:pt-8">
          <div className="mb-6">
            <h1 className="typography-ui-header font-semibold text-foreground">{t('settings.providers.title')}</h1>
            <p className="typography-meta text-muted-foreground mt-1">{t('settings.providers.description')}</p>
          </div>
          <div className="flex h-full items-center justify-center">
            <div className="text-center text-muted-foreground">
              <RiStackLine className="mx-auto mb-3 h-12 w-12 opacity-50" />
              <p className="typography-body">{t('features.providers.selectFromSidebar', 'Select a provider from the sidebar')}</p>
              <p className="typography-meta mt-1 opacity-75">{t('features.providers.reviewDetails', 'Review details and configure auth')}</p>
            </div>
          </div>
        </div>
      </ScrollableOverlay>
      ...
      <div className="mx-auto w-full max-w-3xl p-3 sm:p-6 sm:pt-8">
        ...
        <div className="mb-8">
          <div className="mb-1 px-1 flex items-center justify-between gap-2">
            <h3 className="typography-ui-header font-medium text-foreground">{t('features.providers.authentication')}</h3>
            <ButtonSmall
              variant="outline"
              size="xs"
              className="!font-normal"
            ...
            >
              {showAuthPanel ? t('features.providers.hide') : t('features.providers.reconnect')}
            </ButtonSmall>
          </div>
          ...
          <section className="px-2 pb-2 pt-0">
            {!showAuthPanel ? (
              <div className="flex items-center gap-1.5 py-1.5">
                ...
                <span className="typography-ui-label text-foreground">{t('features.providers.connected')}</span>
                <span className="typography-meta text-muted-foreground ml-1">· {t('features.providers.useReconnect', 'Use Reconnect to update credentials')}</span>
              </div>
            ) : authLoading ? (
              <div className="py-1.5 typography-meta text-muted-foreground">{t('features.providers.loadingAuthMethods', 'Loading authentication methods...')}</div>
            ) : (
              <div className="space-y-4">
                <div className="py-1.5">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-1.5">
                    ...
                    >
                      {authBusyKey === `api:${selectedProvider.id}` ? t('features.providers.saving') : t('features.providers.saveKey')}
                    </ButtonSmall>
                  </div>
                </div>
                        ...
                        <div key={`${selectedProvider.id}-${methodLabel}`} className="space-y-3">
                          ...
                          {isPending && (
                            <div className="flex items-center gap-2 mt-2">
                              ...
                              >
                                {authBusyKey === `oauth-complete:${selectedProvider.id}:${index}` ? t('features.providers.saving') : t('features.providers.complete')}
                              </ButtonSmall>
                            </div>
                          )}
                        </div>
        ...
        <div className="mb-8">
          <div className="mb-1 px-1">
            <h3 className="typography-ui-header font-medium text-foreground">{t('features.providers.connectionDetails')}</h3>
          </div>
          ...
          <section className="px-2 pb-2 pt-0">
            <div className="flex flex-col gap-2 py-1.5 sm:flex-row sm:items-center sm:justify-between sm:gap-8">
              <div className="flex min-w-0 flex-col">
                {selectedSources && (selectedSources.auth.exists || selectedSources.user.exists || selectedSources.project.exists || selectedSources.custom?.exists) ? (
                  <span className="typography-meta text-muted-foreground">
                    {t('features.providers.configuredIn')} {[
                      selectedSources.auth.exists ? t('features.providers.authCredentials') : null,
                      selectedSources.user.exists ? t('features.providers.userConfig') : null,
                      selectedSources.project.exists ? t('features.providers.projectConfig') : null,
                      selectedSources.custom?.exists ? t('features.providers.customConfig') : null,
                    ].filter(Boolean).join(', ')}
                  </span>
                ) : (
                  <span className="typography-meta text-muted-foreground">{t('features.providers.noActiveConfig')}</span>
                )}
              </div>
              ...
              >
                {authBusyKey === `disconnect:${selectedProvider.id}` ? t('features.providers.disconnecting') : t('features.providers.disconnect')}
              </ButtonSmall>
            </div>
          </section>
        </div>
        ...
        <div className="mb-8">
          <div className="mb-1 px-1 flex items-center justify-between gap-2">
            <h3 className="typography-ui-header font-medium text-foreground">
              {t('features.providers.availableModels')}
          ...
          <section className="px-2 pb-2 pt-0">
            <div className="relative mb-2">
              ...
              <Input
                ...
                placeholder={t('features.providers.filterModels')}
                className="h-7 pl-8 w-full"
              />
            </div>

            {filteredModels.length === 0 ? (
              <p className="typography-meta text-muted-foreground py-4 text-center">{t('features.providers.noModelsMatch')}</p>
            ) : (
